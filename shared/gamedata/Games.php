...
class Games {
    ...
    public static function getGamesForLetter($letter,$db) {
        ...
        $criteria .= " AND deprecated = 0 AND type NOT IN ('system')";
        
        return $db->Select("games",null,$criteria,"name");
    }

    public static function getGameLetters($db) {
        $letters = $db->RunStatement("SELECT substr(name,1,1) as letter, COUNT(name) as count FROM games GROUP BY letter ORDER BY letter ASC");
    ...
    public static function printGameCounts($db) {
        $data = $db->RunStatement("SELECT COUNT(DISTINCT games.name) as count, type from games, game_versions ".
                    "WHERE type NOT IN ('system')  AND games.name = game_versions.name group by type"); 
